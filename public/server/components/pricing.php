<div class='bloc' style="<?= dataToStyle($data) ?>">
    <section class="py-<?= $data['padding'] ?? 3 ?> text-<?= $data['titleAlign'] ?? 'center' ?> container"
            ...
            <div class="col-lg-6 col-md-8 mx-auto">
                ...
                <p style='margin-bottom: 0;'>
                    <?php foreach (($data['buttons'] ?? []) as $action): ?>
                        <a href="<?= $action['url'] ?? '#' ?>"
                           class="btn btn-<?= $action['type'] ?? 'primary' ?> my-2"><?= $action['label'] ?? 'Acheter' ?></a>
                    <?php endforeach ?>
                </p>
            </div>
        </div>
            ...
            <?php foreach ($data['prices'] as $item): ?>
                <div class='col'>
                    <div class="card mb-4 box-shadow">
                        ...
                        <div class="card-body">
                            <h1 class="card-title pricing-card-title"><?= $item['price'] ?? 'XX' ?> <small
